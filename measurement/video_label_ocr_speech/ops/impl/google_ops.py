...
from ops.base import Operation, VideoSegmenter, VisualCaptioner, LLMQuery, VideoSplitter

try:
    from google.cloud import videointelligence
except ImportError:
    videointelligence = None  # type: ignore
...
class GoogleVideoIntelligenceFeatureImpl(Operation):
    """
    Video Intelligence 单项能力：LABEL_DETECTION / TEXT_DETECTION(OCR) / SPEECH_TRANSCRIPTION。
    每次 execute 发起一次 annotate_video；返回 timing_prep_sec（含 smart_move 与请求组装）、
    timing_vi_operation_wait_sec（annotate_video 之后同步等待 LRO 完成的墙钟时间）。
    """
    ...
    def __init__(
        ...
        feature_name: str,
        *,
        operation_name_for_path: str,
    ):
        super().__init__(provider, region, storage_bucket)
        self.feature_name = feature_name.upper().strip()
        self.operation_name_for_path = operation_name_for_path
        self._video_client = None

    def _feature_enum(self):
        if videointelligence is None:
            raise ImportError("google-cloud-videointelligence 未安装，请 pip install google-cloud-videointelligence")
        try:
            return getattr(videointelligence.Feature, self.feature_name)
        except AttributeError as e:
            raise ValueError(f"未知的 VI Feature 名称: {self.feature_name}") from e
    ...
    def _summarize(self, result: Any) -> Dict[str, Any]:
        out: Dict[str, Any] = {}
        if not getattr(result, "annotation_results", None):
            return out
        ar = result.annotation_results[0]
        if self.feature_name == "LABEL_DETECTION":
            out["segment_label_count"] = len(ar.segment_label_annotations or [])
            out["shot_label_count"] = len(ar.shot_label_annotations or [])
        elif self.feature_name == "TEXT_DETECTION":
            out["text_annotation_count"] = len(ar.text_annotations or [])
        elif self.feature_name == "SPEECH_TRANSCRIPTION":
            out["speech_transcription_count"] = len(ar.speech_transcriptions or [])
        return out

    def execute(self, video_uri: str, **kwargs) -> Dict[str, Any]:
        feat = self._feature_enum()

        print(f"--- [Google Video Intelligence] feature={self.feature_name} region={self.region} ---")

        target_path = kwargs.get("target_path")
        t_prep0 = time.perf_counter()
        target_uri = self.transmitter.smart_move(
            video_uri, "google", self.storage_bucket, target_path, target_region=self.region
        )
        print(f"    Data Ready: {target_uri}")

        result_filename = kwargs.get("result_filename", "annotations.json")
        result_path = self._build_result_path(
            target_uri, self.operation_name_for_path, result_filename, target_path
        )
        result_uri = f"gs://{self.storage_bucket}/{result_path}"
        ...
        if self.feature_name == "SPEECH_TRANSCRIPTION":
            lang = kwargs.get("speech_language_code", "en-US")
            request_dict["video_context"] = videointelligence.VideoContext(
        ...
        if location_id:
            ...
            raise ValueError(
                f"Unsupported region '{self.region}' for Google Video Intelligence. "
                "Supported regions: us-west1, asia-east1"
            )
        ...
        prep_sec = t_vi0 - t_prep0

        timeout_kw = kwargs.get("annotate_timeout_sec")
        operation = self.video_client.annotate_video(request=request_dict)
        print("    Waiting for video analysis to complete...")
        if timeout_kw is None:
            result = operation.result()
        else:
            result = operation.result(timeout=int(timeout_kw))
        t_vi1 = time.perf_counter()
        vi_wait_sec = t_vi1 - t_vi0
        ...
        return {
            ...
            "source_used": target_uri,
            "result_location": result_uri,
            "timing_prep_sec": round(prep_sec, 6),
            "timing_vi_operation_wait_sec": round(vi_wait_sec, 6),
            **summary,
        }
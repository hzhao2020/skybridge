...
#
# 部署步骤（概要）:
#   1. 本机与云端 VM安装: sudo apt-get update && sudo apt-get install -y iperf3
#   2. 云端 VM 常驻: iperf3 -s -p 5201  （端口与 IPERF_PORT 一致；可用 systemd 托管）
#   3. 云安全组/防火墙: 放行本机出口访问的 TCP(IPERF_PORT)；若需 RTT 则放行 ICMP echo
#   4. 编辑 local_to_cloud_probe.conf 中的 CLOUD_HOST 等
#   5. chmod +x local_to_cloud_probe.sh
#   6. 前台: ./local_to_cloud_probe.sh --config local_to_cloud_probe.conf
#      后台: nohup ./local_to_cloud_probe.sh --config local_to_cloud_probe.conf >> probe.out 2>&1 &
#
...
: "${CLOUD_HOST:?请在配置中设置 CLOUD_HOST（云端可达地址）}"
: "${IPERF_PORT:=5201}"
: "${IPERF_SECONDS:=10}"
: "${PING_COUNT:=10}"
: "${INTERVAL_SEC:=300}"
: "${DURATION:=24h}"
: "${LOG_DIR:=${HOME}/log/local_to_cloud_probe}"
: "${REGION_LOCAL:=local}"
: "${REGION_CLOUD:=cloud}"
: "${CONTINUE_ON_ERROR:=1}"
...
mkdir -p "$LOG_DIR"
_run_id="$(date -u +%Y%m%dT%H%M%SZ)_${REGION_LOCAL}_to_${REGION_CLOUD}"
...

{
  ...
  echo "region_local=${REGION_LOCAL} region_cloud=${REGION_CLOUD}"
  echo "interval_sec=${INTERVAL_SEC} total_sec=${TOTAL_SEC}"
  echo "ping_cmd=ping -n -c ${PING_COUNT} -W 2 ${CLOUD_HOST}"
  echo "iperf_cmd=iperf3 -c ${CLOUD_HOST} -p ${IPERF_PORT} -t ${IPERF_SECONDS} -f m --bidir"
  echo "rtt_csv=${RTT_CSV}"
  echo "bandwidth_csv=${BW_CSV}"
  echo "provenance=${PROVENANCE_FILE}"
} > "${LOG_DIR}/run_${_run_id}.meta"

echo "timestamp_utc,region_local,region_cloud,cloud_host,ping_count,rtt_min_ms,rtt_avg_ms,rtt_max_ms,rtt_mdev_ms,packet_loss_pct,ping_ok,rtt_notes,ping_stdout_stderr" > "$RTT_CSV"
echo "timestamp_utc,region_local,region_cloud,cloud_host,iperf_port,iperf_seconds,bw_out_mbits_per_sec,bw_in_mbits_per_sec,iperf_ok,bw_notes,iperf_stdout_stderr" > "$BW_CSV"
...
log_rtt_row() {
  ...
  n="${n//,/;}"
  {
    printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,' \
      "$ts" "$REGION_LOCAL" "$REGION_CLOUD" "$CLOUD_HOST" "$PING_COUNT" \
...
log_bw_row() {
  ...
  n="${n//,/;}"
  {
    printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,' \
      "$ts" "$REGION_LOCAL" "$REGION_CLOUD" "$CLOUD_HOST" "$IPERF_PORT" "$IPERF_SECONDS" \
      "$bw_out" "$bw_in" "$iok" "$n"
    csv_field "$raw"
    printf '\n'
  } >> "$BW_CSV"
...
probe_once() {
  ...
  set +e
  iout="$(iperf3 -c "$CLOUD_HOST" -p "$IPERF_PORT" -t "$IPERF_SECONDS" -f m --bidir 2>&1)"
  istat=$?
  set -e
